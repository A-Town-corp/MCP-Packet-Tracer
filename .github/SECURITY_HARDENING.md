...
- Code scanning uses GitHub's **default setup**. Keep Python, JavaScript/
  TypeScript, and GitHub Actions selected, choose the `security-extended` query
  suite if the repository's plan offers it, and turn on Copilot Autofix.

Do not add a second checked-in CodeQL analysis workflow while default setup is
enabled. Default setup already creates the dynamic `Analyze` and `Upload
Results` jobs; running advanced setup at the same time causes duplicate CodeQL
analyses and can make the checked-in jobs fail during result upload.
...
- Require these status checks:
  ...
  - `Python 3.13`
  - `Dependency review`
  - The CodeQL default-setup `Analyze` checks for Python and
